...
class Chimera:
    ...
    4. To draw the Chimera graph of the building block, use draw_block().
    >>> chimera.draw_block(graph)
    ...
    def replicate(self, graph, rows=1, columns=1, connector_sampler=np.random.random):
        ...
        Returns
        ---------
        (dict) all biases of the replicated graph
        (dict) all edges of the replicated graph
        """
        ...
        replicated_bias = dict()
        replicated_edge = dict()
        for row in range(rows):
            for column in range(columns):
                for (n1, n2), w in graph.items():
                    # Rescale nodes to correct position
                        ...
                        if 0 <= n1 < 2048 and 0 <= n2 < 2048:
                            edge = tuple(sorted([n1, n2]))
                            if n1 == n2:
                                replicated_bias[n1] = w
                            else:
                                replicated_edge[edge] = w
            
                #randomly sampling inter-block edges
                if row > 0:
                    for n1, n2 in vertical_edges:
                        n1 += (128 * (row * n_rows -1)) + (8 *       column * n_columns)
                        n2 += (128 * (row * n_rows -1)) + (8 * column * n_columns)
                        if 0 <= n1 < 2048 and 0 <= n2 < 2048:
                            edge = tuple(sorted([n1, n2]))
                            replicated_edge[edge] = connector_sampler()

                if column > 0:
                    for n1, n2 in horizontal_edges:
                        n1 += (128 * row * n_rows) + (8 *       (column* n_columns-1))
                        n2 += (128 * row * n_rows) + (8 * (column* n_columns -1))
                        if 0 <= n1 < 2048 and 0 <= n2 < 2048:
                            edge = tuple(sorted([n1, n2]))
                            replicated_edge[edge] = connector_sampler()

        return replicated_bias, replicated_edge
    ...
    def draw_block(self, graph):
        B = self.replicate(graph,1,1)
